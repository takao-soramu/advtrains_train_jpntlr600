...
-- YELLOW
-- TLR0600 <FRONT>
advtrains.register_wagon("Tlr600_yellow_front", {
	mesh="advtrains_tlr600_front.b3d",
	textures = {"advtrains_tlr600_yellow.png"},
	...
	custom_on_step = function(self, dtime)
		...
		if deltatime > 60 then
			...
			if time > 16500 or time < 5500 then
				self.object:set_properties({textures = {"tlr600_yellow_light_front.png"},})
				self.object:set_properties({glow = 5,})
			else
				self.object:set_properties({textures = {"advtrains_tlr600_yellow.png"},})
				self.object:set_properties({glow = 0,})
			end
			deltatime = 0.0
		end
	end,
}, S("Tlr600_yellow_front"), "advtrains_tlr600_yellow_inv.png")

-- TLR0600 <BACK>
advtrains.register_wagon("Tlr600_yellow_back", {
	...
	custom_on_step = function(self, dtime)
		...
		if deltatime > 60 then
			local time = minetest.get_timeofday() * 24000
			if time > 16500 or time < 5500 then
				self.object:set_properties({textures = {"tlr600_yellow_light_back.png"},})
				self.object:set_properties({glow = 5,})
			else
				self.object:set_properties({textures = {"advtrains_tlr600_yellow.png"},})
				self.object:set_properties({glow = 0,})
...
}, S("Tlr600_yellow_back"), "advtrains_tlr600_yellow_inv.png")
